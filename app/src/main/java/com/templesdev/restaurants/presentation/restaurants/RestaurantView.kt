...
fun RestaurantItem(
    ...
    Card(
        ...
        Row(
            ...
            RestaurantDetails(
                item.title, item.description, Modifier.weight(0.7f)
            )
            RestaurantIcon(icon = icon, Modifier.weight(0.15f)) {
...
fun RestaurantDetails(
    ...
    Column(modifier = modifier, horizontalAlignment = horizontalAlignment) {
        Text(
            text = title, style = MaterialTheme.typography.titleMedium, color = Color.DarkGray
        )
...
fun DefaultPreview() {
    RestaurantsAppTheme {
        RestaurantScreen(state = RestaurantState(listOf(), false)) { _, _ -> }
    }
}